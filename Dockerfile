...
# Dependancy Install
RUN apt-get update
RUN apt install -y build-essential libpcap-dev libpcre3-dev libdumbnet-dev python zlib1g-dev liblzma-dev openssl libssl-dev bison flex libhwloc-dev pkg-config openssh-server net-tools iputils-ping
...
WORKDIR /snort
RUN python file_extract.py
RUN python snort_automation.py